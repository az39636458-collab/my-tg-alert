...
const POSITIONS_KEY = 'active_trading_positions'; // 🚀 新增：Redis 倉位記憶 Key
...
async function monitorPosition(positionId, symbol, originalQty, entryPrice) {
    console.log(`👁️ 開始監控: ${symbol} | positionId=${positionId}`);
    ...
    const interval = setInterval(async () => {
        try {
            ...
            if (!pos) {
                ...
                await redis.hdel(POSITIONS_KEY, positionId); // 同步刪除 Redis 記憶
                clearInterval(interval);
                return;
            }
            ...
            if (posInfo.phase === 1 && currentQty < originalQty * 0.9) {
                ...
                if (posInfo.tp2Price) {
                    const res = await bitunix.placeTpSl(
                        ...
                        entryPrice, currentQty // 止損上移到開倉價
                    );
                    if (res.code === 0) {
                        console.log(`✅ 階段2掛單成功：TP2=${posInfo.tp2Price} | SL=${entryPrice}(開倉價)`);
                        posInfo.tp3Qty = tp3Qty;
                        await redis.hset(POSITIONS_KEY, positionId, JSON.stringify(posInfo)); // 更新 Redis 狀態
                    }
                }
            }

            // ===== 止盈二成交 (由 Phase 2 轉 Phase 3) =====
                ...
                if (posInfo.tp3Price) {
                    const res = await bitunix.placeTpSl(
                        ...
                        posInfo.tp1Price, currentQty // 止損上移到 TP1 價格
                    );
                    if (res.code === 0) {
...
async function executeOrder(messageText) {
    ...
    try {
        ...
        const posData = {
            ...
            totalQty, tp1Qty, tp1Price: parseFloat(takeProfit1Match[1]),
        ...
        await redis.hset(POSITIONS_KEY, pos.positionId, JSON.stringify(posData)); // 🚀 存入 Redis
...
async function startBot() {
    ...
    await recoverPositions(); // 🚀 開機第一件事：找回記憶
